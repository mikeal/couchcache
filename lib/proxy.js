...
CacheMachine.prototype.startListener = function (pathname) {
  ...
  if (dbname[0] !== '_') {
    if (!this.listeners[dbname]) {
      ...
      var changes = couchdb.createClient(this.options.uri.port, this.options.uri.hostname).db(dbname).changesStream()
      changes.addListener("change", listener);
      changes.addListener("end", function () {
        delete this.listeners[dbame];
      });
      this.listeners[dbname] = listener;
    }
  }
}

CacheMachine.prototype.invalidate = function (dbname, change) {
  var key = dbame+'/'+change['id'];
  this.cache.unset(key)
  if (this.cache.hasKey('index/'+key)) {
    var index = this.cache.get('index/'+key);
    if (index.aliases) {
      index.aliases.forEach(function (a) {this.cache.unset(key)})
    }
  }
}
...
CacheMachine.prototype.setAlias = function (key) {
  if (key.indexOf('?') !== -1) {
    if (this.cache.hasKey('index/'+key.split('?')[0])) {
      var index = this.cache.get('index/'+key.split('?')[0])
      index.push(key); 
...
CacheMachine.prototype.getKey = function (pathname) {
  ...
  if (p.length == 2) {
    if (p[0][0] !== '_' && p[1][0] !== '_') {
      sys.puts(pathname + ' ' + JSON.stringify(p))
      var key = p[0] + '/' + p[1];
      this.setAlias(key); 
      return key;
    }
